...
pub(crate) fn compute_species_colors(registry: &fluidsim::SpeciesRegistry) -> Vec<[f32; 4]> {
    ...
    // Golden-ratio hue stepping spreads auto-assigned colors evenly around the
    // wheel; the reserved band keeps them clear of the cyan used for fluid.
    const GOLDEN_RATIO_CONJUGATE: f32 = 0.618_034;
    const RESERVED_HUE: f32 = 0.54;
    const MIN_HUE_DISTANCE: f32 = 0.12;
    const HUE_NUDGE: f32 = 0.17;

    let boost_contrast = |channel: f32| 0.5 + (channel - 0.5) * 1.1;
    ...
    registry
        ...
        .map(|info| {
            ...
            let mut hue = (info.index as f32 * GOLDEN_RATIO_CONJUGATE).fract();
            for _ in 0..8 {
                let delta = (hue - RESERVED_HUE).abs();
                let distance = delta.min(1.0 - delta);
                if distance >= MIN_HUE_DISTANCE {
                    break;
                }
                hue = (hue + HUE_NUDGE).fract();
            }

            let rgb = hue_to_rgb(hue);
            [
                boost_contrast(rgb[0]),
                boost_contrast(rgb[1]),
                boost_contrast(rgb[2]),
                1.0,
            ]
        })
        .collect()
