...
impl DemoApp {
    ...
    pub(crate) fn draw_enzymes(&self, ctx: &egui::Context) {
        ...
        for entity in sim.enzyme_entities() {
            ...
            const SHELL_SEGMENTS: usize = 36;
            let mut shell = Vec::with_capacity(SHELL_SEGMENTS);
            for segment in 0..SHELL_SEGMENTS {
                let angle = segment as f32 / SHELL_SEGMENTS as f32 * std::f32::consts::TAU;
