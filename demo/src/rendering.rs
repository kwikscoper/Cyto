...
use crate::app_types::{RenderFrameMetrics, duration_millis_f32};
...
impl DemoApp {
    pub(crate) fn render_frame(&mut self) -> Result<RenderFrameMetrics> {
        ...
        let Some(image_index) = ctx.begin_frame()? else {
        ...
        Ok(RenderFrameMetrics {
            upload_ms: duration_millis_f32(t1 - t0),
            render_ms: duration_millis_f32(t3 - t1),
        })
    }
