...
/// Which preset scenario the simulation starts in.
...
/// User-facing swapchain present-mode preference, mapped onto the renderer's
...
#[derive(Parser)]
#[command(name = "cyto-demo", about = "Fluid simulation demo")]
pub(crate) struct Cli {
    ...
    /// Swapchain present mode: auto, fifo, or mailbox.
    ///
    /// `auto` requests low-latency mailbox where the platform/driver exposes it
    /// and otherwise falls back to fifo (vsync). On capture-sensitive setups
    /// (e.g. a compositor recording via OBS/PipeWire on X11) auto stays on the
    /// fifo path for smoother frames; this is harmless on Wayland, Windows, and
    /// macOS/MoltenVK. Use `fifo` to force vsync or `mailbox` to prefer low
    /// latency.
