/// Maximum species rows rendered in a detail probe before collapsing the rest.
const MAX_SPECIES_ROWS: usize = 4;
/// Maximum material rows rendered in a detail probe.
const MAX_MATERIAL_ROWS: usize = 2;
...
pub(crate) fn format_detail_probe_tooltip(result: &fluidsim::InspectionResult) -> String {
    ...
    if !result.species.is_empty() {
        ...
        for entry in result.species.iter().take(MAX_SPECIES_ROWS) {
            lines.push(format!(
                "  {:<10} {:>7.3} M",
                entry.name, entry.concentration
            ));
        }
        if result.species.len() > MAX_SPECIES_ROWS {
            lines.push(format!("  +{} more", result.species.len() - MAX_SPECIES_ROWS));
        }
    }

    if !result.materials.is_empty() {
        lines.push("Materials:".to_string());
        for material in result.materials.iter().take(MAX_MATERIAL_ROWS) {
            lines.push(format!(
                "  {} {:>5.0}%",
                material.name,
