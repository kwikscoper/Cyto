...
impl KineticsConfig {
    ...
    /// Create a minimal no-op configuration: Lean disabled, every rule family
    /// off, conservation checks off, and a tight evaluation budget.
    pub fn noop() -> Self {
        Self {
            enable_lean: false,
    ...
    /// Whether any semantic rule family is enabled.
    ///
    /// When this is true the engine must have a Lean backend, since Lean is
    /// the single source of truth for active rules.
    pub fn any_rules_enabled(&self) -> bool {
        self.enable_diffusion_rules
