...
impl KineticsEngine {
    ...
    pub fn new(config: KineticsConfig) -> Result<Self, KineticsError> {
        ...
        let evaluator: Box<dyn RuleEvaluator> = if config.any_rules_enabled() {
    ...
    pub fn run_diagnostics(&self, snapshot: &SemanticSnapshot) -> Vec<KineticsDiagnostic> {
        ...
        // Conservation accounting is performed on the GPU side in `fluidsim`;
        // here we only emit a marker so callers can see the check ran.
        if self.config.enable_conservation_checks {
            diagnostics.push(KineticsDiagnostic::info(
                "Conservation check delegated to fluidsim".to_string(),
            ));
        }

        // Flag physically impossible per-tile values (negative T or molarity).
