...
/// Multipliers within this distance of `1.0` (and additive terms within this
/// distance of `0.0`) are treated as having no effect.
const NOOP_EPSILON: f64 = 1e-9;
...
impl BoundaryUpdate {
    ...
    /// Check if this update has any effect.
    pub fn is_noop(&self) -> bool {
        (self.permeability_multiplier - 1.0).abs() < NOOP_EPSILON
            && self.transport_mode == BoundaryTransportMode::Normal
            && self.species_specific_permeability.is_empty()
            && (self.thermal_conductance_multiplier - 1.0).abs() < NOOP_EPSILON
    }
}
...
impl SemanticUpdate {
    ...
    pub fn is_noop(&self) -> bool {
        self.tile_updates.iter().all(|u| u.is_noop())
            ...
            && (self.global_diffusion_multiplier - 1.0).abs() < NOOP_EPSILON
            && self.global_temperature_adjustment.abs() < NOOP_EPSILON
    }
