...
/-- Check whether a single species is present in a tile above `minMolarity`. -/
...
/-- Check whether every named species is present in a tile above `minMolarity`. -/
def tileHasAllSpecies (tile : TileSnapshot) (speciesNames : Array String) : Bool :=
  speciesNames.all (tileHasSpecies tile)

/-- Ids of fluid tiles that contain all of `speciesNames`. -/
...
def makeRule
    (reactionName reactantA reactantB productA productB : String)
...
/-- Append `rule` only when it applies to at least one tile. -/
def pushRuleIfAny
    (rules : Array ReactionRule)
    (tileIds : Array Nat)
    (rule : ReactionRule) : Array ReactionRule :=
