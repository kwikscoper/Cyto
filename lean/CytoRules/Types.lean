/-
  ...
  Lean uses camelCase fields internally, but every JSON key is snake_case so
  the codecs line up exactly with the serde structs in the `kinetics` crate.
  Keep the JSON keys below in sync with those structs.
-/
