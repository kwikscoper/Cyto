...
fn infer_ionic_charge(name: &str) -> i32 {
    parse_parenthesized_charge(name)
        .or_else(|| parse_suffix_charge(name))
        .unwrap_or(0)
}

/// Combine a `+`/`-` sign character with a magnitude into a signed charge.
fn signed_charge(sign: char, magnitude: i32) -> i32 {
...
fn parse_suffix_charge(name: &str) -> Option<i32> {
    ...
    // Trailing ASCII digits are single-byte, so slicing on the count is safe.
    ...
    let magnitude = if digit_count == 0 {
        1
    } else {
        prefix[prefix.len() - digit_count..].parse::<i32>().ok()?
    };
    Some(signed_charge(sign, magnitude))
}
