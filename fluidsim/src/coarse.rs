...
//! - Rate-limited reads (governed by `min_read_interval`, default 200ms)
//!
...
impl CoarseGrid {
    ...
    /// Rate-limited by `min_read_interval`; returns `false` when throttled,
    /// when a readback is already in flight, or for out-of-bounds coordinates.
    ...
    pub fn poll_readback(&mut self) -> Option<CoarseCellData> {
        match &self.readback_state {
            ...
            ReadbackState::Pending {
                ...
                if status == Ok(true) {
                    // Fence signaled: the copy finished, so the mapped readback
                    // buffer now holds this cell's data.
                    let _ = submit_time;
