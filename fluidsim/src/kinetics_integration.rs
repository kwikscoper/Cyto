...
impl SemanticUpdateApplicator {
    ...
    pub fn apply(
        ...
        Self::log_update_diagnostics(update);
    ...
    /// Forward kinetics diagnostics to the `log` facade at their severity.
    fn log_update_diagnostics(update: &SemanticUpdate) {
        use kinetics::diagnostics::DiagnosticLevel;
        for diagnostic in &update.diagnostics {
            match diagnostic.level {
                DiagnosticLevel::Error => log::error!("Kinetics: {}", diagnostic),
                DiagnosticLevel::Warning => log::warn!("Kinetics: {}", diagnostic),
                DiagnosticLevel::Info => log::info!("Kinetics: {}", diagnostic),
                DiagnosticLevel::Debug => log::debug!("Kinetics: {}", diagnostic),
            }
        }
    }
