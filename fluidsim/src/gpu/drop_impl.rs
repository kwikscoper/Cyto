...
impl Drop for GpuSimulation {
    // Teardown order matters: wait for the device to go idle, drop the coarse
    // grid (which holds borrowed Vulkan handles) first, then destroy the
    // optional pipelines, then the core buffers, and finally the owned context.
    fn drop(&mut self) {
        log::debug!("GpuSimulation::drop - starting");
        unsafe {
            self.device.device_wait_idle().ok();
            log::debug!("GpuSimulation::drop - device idle");

            drop(self.coarse_grid.take());
            log::debug!("GpuSimulation::drop - coarse_grid dropped");
            ...
            log::debug!("GpuSimulation::drop - reaction pipeline dropped");
            ...
            log::debug!("GpuSimulation::drop - leak pipeline dropped");
            ...
            log::debug!("GpuSimulation::drop - enzyme pipeline dropped");
            ...
            drop(alloc);
            log::debug!("GpuSimulation::drop - buffers freed");
            ...
            if self.owns_vulkan_context {
                ...
                log::debug!("GpuSimulation::drop - owned Vulkan context destroyed");
            } else {
                log::debug!("GpuSimulation::drop - shared Vulkan context left alive");
            }
            log::debug!("GpuSimulation::drop - done");
        }
    }
}
