...
impl GpuSimulation {
    ...
    /// Compute a stable fingerprint of a reaction rule set so repeated uploads
    /// of an unchanged set can be skipped.
    fn hash_reaction_rules(rules: &[GpuReactionRule]) -> u64 {
    ...
    pub fn upload_reaction_rules(&mut self, rules: &[GpuReactionRule]) -> Result<bool> {
        ...
        let new_hash = Self::hash_reaction_rules(rules);
