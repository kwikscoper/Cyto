...
impl GpuBuffer {
    /// Allocate a Vulkan buffer of `size` bytes and bind it to fresh memory
    ...
    /// Copy `data` into the start of this buffer's host-visible mapping.
    ...
    /// Read the first `count` elements of type `T` from the host-visible mapping.
