...
/// Rank physical devices so a discrete GPU is preferred over integrated,
/// virtual, and software/CPU fallbacks.
fn physical_device_priority(device_type: vk::PhysicalDeviceType) -> u32 {
    match device_type {
...
impl GpuSimulation {
    /// Create a simulation that owns its own Vulkan instance, device, and
    /// allocator. Suitable for headless probes and tests.
    ...
    /// Create a simulation that borrows an existing Vulkan context (e.g. one
    /// shared with a windowed renderer). The context is left intact on drop.
    ...
    fn create_owned_context() -> Result<(ash::Entry, SharedGpuContext)> {
        ...
        let physical_device = physical_devices
            ...
            .max_by_key(|&pd| {
                ...
                physical_device_priority(props.device_type)
            })
            .unwrap();
