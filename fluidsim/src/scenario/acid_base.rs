...
pub fn create_acid_base_scenario(width: u32, height: u32) -> Scenario {
    ...
    for y in bounds.inner_y0..bounds.inner_y1 {
        for x in bounds.inner_x0..bounds.inner_x1 {
            ...
            // Split the interior diagonally: acid on the top-left, base on the
            // bottom-right, so the two solutions mix along the diagonal.
            let (nx, ny) = normalized_position(bounds, x, y);
            if nx + ny < 1.0 {
                builder.seed_cell_concentration(index, h_id, 1.0);
                builder.seed_cell_concentration(index, so4_id, 0.5);
            } else {
                builder.seed_cell_concentration(index, na_id, 1.0);
                builder.seed_cell_concentration(index, oh_id, 1.0);
            }
        }
    }
