...
pub fn create_catalyst_scenario(width: u32, height: u32) -> Scenario {
    ...
    for y in bounds.inner_y0..bounds.inner_y1 {
        for x in bounds.inner_x0..bounds.inner_x1 {
            ...
            builder.seed_cell_concentration(index, glucose_id, 1.0);
            builder.seed_cell_concentration(index, atp_id, 1.0);
            builder.seed_cell_concentration(index, hexokinase_id, 0.01);

            // Warm the top-left half to bias the catalysed reaction there.
