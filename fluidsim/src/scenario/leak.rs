...
pub fn create_leak_scenario(width: u32, height: u32) -> Scenario {
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let inside_box = x >= bounds.inner_x0
                ...
                && y < bounds.inner_y1;
            if !inside_box {
                // Exterior bath: KCl surrounding the membrane box.
                builder.seed_cell_concentration(index, k_id, 1.0);
                builder.seed_cell_concentration(index, cl_id, 1.0);
            }
        }
    }
    ...
    for y in bounds.inner_y0..bounds.inner_y1 {
        for x in bounds.inner_x0..bounds.inner_x1 {
            ...
            if nx + ny < 1.0 {
                builder.seed_cell_concentration(index, na_id, 0.35);
                builder.seed_cell_concentration(index, h_id, 1.8e-5);
                builder.seed_cell_concentration(index, acetate_id, 0.350018);
                builder.seed_cell_concentration(index, acetic_acid_id, 0.349982);
            } else {
                builder.seed_cell_concentration(index, na_id, 0.2);
                builder.seed_cell_concentration(index, oh_id, 0.2);
