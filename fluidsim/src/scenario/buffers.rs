...
pub fn create_buffers_scenario(width: u32, height: u32) -> Scenario {
    ...
    for y in bounds.inner_y0..bounds.inner_y1 {
        for x in bounds.inner_x0..bounds.inner_x1 {
            ...
            if nx + ny < 1.0 {
                // Top-left: acetate/acetic-acid buffer at its weak-acid equilibrium.
                builder.seed_cell_concentration(index, na_id, 0.35);
                builder.seed_cell_concentration(index, h_id, 1.8e-5);
                builder.seed_cell_concentration(index, acetate_id, 0.35);
                builder.seed_cell_concentration(index, acetic_acid_id, 0.35);
            } else {
                // Bottom-right: dilute NaOH titrant.
