...
use crate::species::{SpeciesConcentrations, SpeciesId, SpeciesRegistry};
use ahash::AHashMap;
...
/// Default cell temperature (20 °C) applied when a scenario sets none.
const DEFAULT_TEMPERATURE_KELVIN: f32 = 293.15;
...
impl ScenarioBuilder {
    ...
    /// Seed the concentration of a known species at a specific cell index.
    ///
    /// Shared by the scenario constructors, which already hold resolved
    /// `SpeciesId`s and the linear cell index.
    pub(crate) fn seed_cell_concentration(
        &mut self,
        index: usize,
        species: SpeciesId,
        concentration: f64,
    ...
    pub fn build(self) -> Scenario {
        let temperatures = if self.initial_temperatures.is_empty() {
            vec![DEFAULT_TEMPERATURE_KELVIN; self.grid.cell_count()]
        } else {
