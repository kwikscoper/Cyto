...
fn main() {
    // Charge projection is disabled here so the probe can verify the leak
    // transport stays locally electroneutral on its own; the rest matches the
    // library defaults aside from the smaller grid and 60 Hz frame cap.
