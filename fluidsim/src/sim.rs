...
/// Leak-channel scenarios rely on transport staying locally electroneutral on
/// its own, so the global charge-projection pass is disabled when any channels
/// are present to avoid fighting the channel dynamics.
...
impl Simulation {
    ...
    pub fn request_async_inspection(&mut self, screen_x: f32, screen_y: f32) -> bool {
        ...
        let Some(coarse) = self.gpu.coarse_grid.as_mut() else {
            return false;
        };
    ...
    pub fn poll_async_inspection(&mut self) -> Option<CoarseCellData> {
        ...
        self.gpu.coarse_grid.as_mut()?.poll_readback()
    }
    ...
    pub fn get_cached_inspection(&self) -> Option<CoarseCellData> {
        ...
        self.gpu.coarse_grid.as_ref()?.get_cached_cell()
    }
    ...
    pub fn is_inspection_pending(&self) -> bool {
        self.gpu
            .coarse_grid
            .as_ref()
            .is_some_and(|coarse| coarse.is_readback_pending())
    }
    ...
    pub fn time_since_kinetics_evaluation(&self) -> f64 {
        self.kinetics
            .as_ref()
            .map_or(0.0, |k| k.time_since_last_evaluation())
    }
