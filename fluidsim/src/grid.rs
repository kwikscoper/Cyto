...
impl Grid {
    ...
    /// Get the 4-connected (von Neumann) neighbors of a cell, in
    /// left/right/top/bottom order, skipping any that fall outside the grid.
    pub fn neighbors(&self, coord: CellCoord) -> impl Iterator<Item = CellCoord> {
        let CellCoord { x, y } = coord;
        let mut neighbors = Vec::with_capacity(4);
        if x > 0 {
            neighbors.push(CellCoord::new(x - 1, y));
        }
        if x + 1 < self.width {
            neighbors.push(CellCoord::new(x + 1, y));
        }
        if y > 0 {
            neighbors.push(CellCoord::new(x, y - 1));
        }
        if y + 1 < self.height {
            neighbors.push(CellCoord::new(x, y + 1));
        }
