    ...
    fn aggregate_tile(
        ...
        if fluid_count > 0 {
            ...
            for (s, info) in input.species_registry.iter().enumerate() {
                ...
                let mean = species_sums[s] / fluid_count_f;
                tile.species_mean_molarity
                    .push(SpeciesAmount::new(species_id, mean));

                // Total moles, assuming each cell is one unit of volume.
                tile.species_total_moles
                    .push(SpeciesAmount::new(species_id, species_sums[s]));

                // Gradient magnitude approximated from the per-tile min/max spread.
