...
/// Absolute tolerance (in summed molarity) for the per-species mass- and
/// charge-conservation checks.
const CONSERVATION_TOLERANCE: f32 = 0.1;

/// Parse the probe's `key=value` lines into a metric map, ignoring any line
/// that is not a parseable `name=float` pair.
