...
//!
//! Renders tooltip and UI overlays on top of the simulation using
//! `egui-ash-renderer`, driving input with `egui-winit`.
...
pub struct EguiRenderer {
    ...
    /// Tessellated output captured in `end_frame`, consumed during recording.
...
impl EguiRenderer {
    pub fn new(render_ctx: &RenderContext, window: &Window) -> Result<Self> {
        ...
        // The default allocator keeps egui's GPU memory separate from
        // `RenderContext`'s gpu-allocator, avoiding any cross-locking.
        let renderer = Renderer::with_default_allocator(
            ...
            Options {
                // Keep egui's existing gamma handling unchanged (see note in
                // the crate: the swapchain itself prefers an sRGB format).
