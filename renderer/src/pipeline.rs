...
/// Number of storage-buffer bindings consumed by the visualization shader
/// (concentrations, solid mask, material ids, species colors, temperatures).
const STORAGE_BINDING_COUNT: u32 = 5;
...
impl RenderPipeline {
    pub fn new(
        ...
        // Descriptor set layout: bindings 0..=4 are all fragment-stage storage
        // buffers (concentrations, solid mask, material ids, species colors,
        // temperatures), so build them uniformly.
        let bindings: Vec<vk::DescriptorSetLayoutBinding<'_>> = (0..STORAGE_BINDING_COUNT)
            .map(|binding| {
        ...
        let pool_sizes = [vk::DescriptorPoolSize::default()
            ...
            .descriptor_count(STORAGE_BINDING_COUNT)];
        ...
        // Point the descriptor set at the renderer-owned buffers; later calls to
        // `bind_simulation_buffers` may rebind bindings 0/1/2/4 to GPU buffers.
        update_storage_descriptors(
            &ctx.device,
            descriptor_set,
            &[
                (concentration_buffer.buffer, conc_size),
                (solid_mask_buffer.buffer, mask_size),
                (material_id_buffer.buffer, mask_size),
                (species_color_buffer.buffer, color_size),
                (temperature_buffer.buffer, temp_size),
            ],
        );
    ...
    pub fn bind_simulation_buffers(&mut self, ctx: &RenderContext, buffers: GpuRenderBuffers) {
        ...
        // Bindings 0/1/2/4 follow the simulation's GPU buffers; binding 3
        // (species colors) stays on the renderer-owned, host-uploaded buffer.
        update_storage_descriptors(
            &ctx.device,
            self.descriptor_set,
            &[
                (buffers.concentration, conc_size),
                (buffers.solid_mask, mask_size),
                (buffers.material_ids, mask_size),
                (self.species_color_buffer.buffer, color_size),
                (buffers.temperature, temp_size),
            ],
        );
    ...
    pub fn upload_state(&mut self, ctx: &RenderContext, state: &RenderState) -> Result<()> {
        // Flatten the per-species concentration slices into one contiguous buffer.
        ...
        // Periodically log a center sample to confirm data keeps updating.
    ...
    pub fn record(
        ...
        // Buffer-transfer barriers are issued in `copy_buffer_batch_with_barriers`,
        // so recording here only needs the draw commands.
        ...
        let push_constants = VisualizationPushConstants {
            ...
            thermal_view: u32::from(thermal_view),
        };
...
/// Rebind the visualization descriptor set's storage buffers.
///
/// Each entry is a `(buffer, range)` pair bound to the binding equal to its
/// index, mirroring `STORAGE_BINDING_COUNT` fragment-stage storage buffers.
