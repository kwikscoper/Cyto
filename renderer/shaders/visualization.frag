...
// Fragment shader for fluid-simulation visualization.
//
// Coloring scheme:
// - Pure water (no solute): deep blue base color
// - Each species uses a pre-computed color from a CPU-side lookup table
// - Low concentrations are boosted nonlinearly so transient streams stay visible
// - Species colors are blended with weighting biased toward the strongest streak
...
void main() {
    ...
    // Animated debug border: a rotating-hue frame proving the view is live.
    float edge_dist = min(min(fragUV.x, 1.0 - fragUV.x), min(fragUV.y, 1.0 - fragUV.y));
    float border_width = 0.01;  // 1% of the screen on each edge
